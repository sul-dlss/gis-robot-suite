    ...
    module GisDelivery   # This is your workflow package name (using CamelCase)
      ...
      class LoadRaster # This is your robot name (using CamelCase)
        ...
        def perform(druid)
          ...
          raise RuntimeError, "load-raster: #{druid} cannot locate MODS: #{modsfn}" unless File.size?(modsfn)
          ...
          raise RuntimeError, "load-raster: #{druid} cannot locate normalized data: #{zipfn}" unless File.size?(zipfn)
